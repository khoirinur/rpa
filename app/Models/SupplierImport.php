...
class SupplierImport extends Model
{
    ...
    {
        return $this->belongsTo(SupplierCategory::class, 'fallback_supplier_category_id');
    }
    ...
    public function process(): void
    {
        ...
        try {
            ...
            $this->existingCodes = Supplier::withTrashed()
                ->pluck('code')
            ...
            foreach ($rows as $index => $row) {
                try {
                    ...
                    $failed++;
                    $identifier = $row['raw_code'] ?: ($row['name'] ?? 'tanpa kode');
                    $logs[] = sprintf('[%s] %s', $identifier, $throwable->getMessage());
                }
            }
    ...
    protected function readCsv(): array
    {
        ...
        while (($data = fgetcsv($stream)) !== false) {
            ...
            $rows[] = [
                ...
                'type' => $values[2] ?? null,
                'name' => $values[3] ?? null,
                'owner_name' => $values[4] ?? null,
                'contact_phone' => $values[5] ?? null,
                'bank_account_name' => $values[6] ?? null,
                'bank_name' => $values[7] ?? null,
                'bank_account_number' => $values[8] ?? null,
                'address_line' => $values[9] ?? null,
            ];
        }
    ...
    protected function importRow(array $row, int $index): void
    {
        ...
        if (! $categoryId) {
            if ($this->fallback_supplier_category_id) {
                $categoryId = $this->fallback_supplier_category_id;
        ...
        Supplier::updateOrCreate(
            ...
            [
                ...
                'owner_name' => $this->normalizeNullableString($row['owner_name'] ?? null),
                'contact_phone' => $this->normalizePhoneString($row['contact_phone'] ?? null),
                'bank_account_name' => $this->normalizeNullableString($row['bank_account_name'] ?? null),
                'bank_name' => $this->normalizeNullableString($row['bank_name'] ?? null),
                'bank_account_number' => $this->normalizeNullableString($row['bank_account_number'] ?? null),
                'address_line' => $this->normalizeNullableString($row['address_line'] ?? null),
    ...
    protected function buildNotes(array $row): ?string
    {
        ...
        if (blank($type)) {
            return 'Import dari supplier.csv';
        }

        return sprintf('Import supplier segmen %s dari supplier.csv', $type);
    }
    ...
    protected function hydrateCategoryMap(): void
    {
        ...
        SupplierCategory::query()
            ...
            ->get()
            ->each(function (SupplierCategory $category) use (&$map): void {
