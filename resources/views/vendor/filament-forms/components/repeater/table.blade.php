@php
    ...
    $rowClickAction = $getExtraAttributes()['data-row-click-action'] ?? null;
    $rowTriggerOnlyAttribute = 'data-row-trigger-only';

    $hasVisibleExtraItemActions = false;

    foreach ($items as $itemKey => $item) {
        foreach ($extraItemActions as $extraItemAction) {
            $actionInstance = $extraItemAction(['item' => $itemKey]);

            if (
                $actionInstance->isVisible() &&
                blank($actionInstance->getExtraAttributes()[$rowTriggerOnlyAttribute] ?? null)
            ) {
                $hasVisibleExtraItemActions = true;
    ...
    $shouldShowActionColumn = $hasVisibleExtraItemActions || $isCloneable || $isDeletable;
@endphp
                    ...
                    >
                        @foreach ($items as $itemKey => $item)
                            @php
                                $visibleExtraItemActions = array_filter(
                                    ...
                                    function (Action $action) use ($itemKey, $rowTriggerOnlyAttribute): bool {
                                        $actionInstance = $action(['item' => $itemKey]);

                                        if (filled($actionInstance->getExtraAttributes()[$rowTriggerOnlyAttribute] ?? null)) {
                                            return false;
                                        }

                                        return $actionInstance->isVisible();
                                    },
                                );
                            ...
                            <tr
                                ...
                                @class([
                                    'cursor-pointer hover:bg-gray-50' => filled($rowClickAction),
                                ])
                                @if ($rowClickAction)
                                    x-data="{}"
                                    x-on:click="
